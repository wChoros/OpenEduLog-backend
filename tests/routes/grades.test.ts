...
const getRouteHandlers = (router: any, method: string, path: string): Function[] => {
  const layer = router.stack.find(
    (l: ExpressLayer) => l.route && l.route.path === path && l.route.methods && l.route.methods[method.toLowerCase()]
  );
  if (!layer || !layer.route) {
    // Corrected the map function's parameter 'l_map' to avoid conflict and add type
    const availableRoutes = router.stack
        .map((l_map: ExpressLayer) => l_map.route && ({ path: l_map.route.path, methods: Object.keys(l_map.route.methods).filter(m => l_map.route!.methods[m]) }))
        .filter(Boolean); // Filter out undefined entries if a layer has no route
    throw new Error(`Handlers not found for ${method.toUpperCase()} ${path}. Available routes: ${JSON.stringify(availableRoutes)}`);
  }
...
describe('Grades Router', () => {
  ...
  describe('GET /grades/details/:gradeId', () => {
    it('should fetch and return grade details', async () => {
      ...
      const mockGradeDetailData = {
        id: 1, value: 5, description: 'Excellent', weight: 2,
        subjectOnTeacher: {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        id: 1, value: 5, description: 'Excellent', weight: 2,
        subjectName: 'History', subjectId: 101,
        teacherFirstName: 'Jane', teacherLastName: 'Doe', teacherId: 201,
        addedAt: mockGradeDetailData.createdAt, updatedAt: mockGradeDetailData.updatedAt,
      });
    });
  ...
  describe('POST /grades/:studentId/:teacherOnSubjectId/:value', () => {
    it('should add a new grade', async () => {
      mockReq = {
        params: {
          studentId: '1', teacherOnSubjectId: '2', value: '5',
          description: 'Good work', weight: '2', // These are from params as per router code
        },
      };
      mockPrismaClientInstance.grade.create.mockResolvedValue({ id: 1 });

      const handlers = getRouteHandlers(gradesRouter, 'post', '/:studentId/:teacherOnSubjectId/:value');
      ...
      expect(mockPrismaClientInstance.grade.create).toHaveBeenCalledWith({
        data: {
          studentId: 1, subjectOnTeacherId: 2, value: 5,
          description: 'Good work', weight: 2,
        },
      });
